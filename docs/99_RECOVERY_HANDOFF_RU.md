...
Phase 0, 1, 2, 3, 4 и 5 — **COMPLETE**. Текущая работа — **Phase 6: Fastboot
DATA IN + modern partition workflows**. Legacy/A2 frozen.

Модулей восемь: `:app`, `:core:model`, `:core:diagnostics`, `:core:operation`,
`:usb:api`, `:usb:android`, `:protocol:adb`, `:protocol:fastboot`. Тестов 677 (`@Test` в текущем дереве).
