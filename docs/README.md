...
Порядок чтения задан в `/CLAUDE.md` — единственном месте, где он живёт. Этот файл — оглавление, а не маршрут.
...
- `0006_FASTBOOT_SINGLE_LANE_AND_THREE_STATE_ROLE_RU.md` — **ACCEPTED** 2026-09-12: Fastboot не мультиплексирует, поэтому полоса обмена одна; роль устройства имеет три состояния, и `UNKNOWN` не сворачивается в «загрузчик». Принято прогоном `07` §6.70: подпись дескриптора в обеих ролях одна и та же, а ответы разные.
